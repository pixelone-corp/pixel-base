...
const InputContainer = styled.div<{ size?: string; display?: string }>`
  &&::after {
    position: absolute;
    width: 25px;
    height: ${(props) => (props.size === 'sm' ? '20px !important' : '25px')};
    content: '';
    ...
    top: ${(props) => (props.size === 'sm' ? '8px !important' : '13px')};
    left: 10px;
    display: ${(props) => (props.display === 'true' ? 'block' : 'none')};
  }
`
const DCInput = styled.input<{ props }>`
  padding: ${(props) =>
