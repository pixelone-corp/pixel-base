...
export function getProperty<T>(
  ...
  selector: Selector<T> | undefined | null | string | ((row: T) => unknown),
  ...
  if (typeof selector === 'function') {
    return selector(row, rowIndex) as React.ReactNode
  } else if (typeof selector === 'string') {
    return (row as any)[selector] as React.ReactNode
  }
...
export function getSortDirection(
  ascDirection: boolean | undefined = false
): SortOrder {
...
export function findColumnIndexById<T>(
  columns: TableColumn<T>[],
  id: string | undefined
): number {
  if (!id) {
